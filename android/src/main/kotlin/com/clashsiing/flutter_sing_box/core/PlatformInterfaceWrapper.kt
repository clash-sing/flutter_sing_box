...
interface PlatformInterfaceWrapper : PlatformInterface {
    ...
    override fun findConnectionOwner(
        ...
        try {
            val uid = PluginManager.connectivity.getConnectionOwnerUid(
                ipProtocol,
    ...
    override fun uidByPackageName(packageName: String): Int {
        return try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                PluginManager.packageManager.getPackageUid(
                    packageName, PackageManager.PackageInfoFlags.of(0)
                )
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                PluginManager.packageManager.getPackageUid(packageName, 0)
            } else {
                PluginManager.packageManager.getApplicationInfo(packageName, 0).uid
            }
